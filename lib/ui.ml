...
let init_model packages =
  ...
  in
  let height =
    match Terminal_size.get_rows () with Some h -> h | None -> 24
  in
  {
    packages;
...
let adjust_scroll_offset model =
  ...
  let scroll_offset =
    if model.selected_idx < model.scroll_offset then model.selected_idx
...
let render_details model =
  let selected_pkg =
    ...
    with _ ->
      { Opam_client.name = ""; version = ""; synopsis = ""; installed = false }
  in
  let open Opam_client in
  ...
  let status_attr =
    if selected_pkg.installed then A.(fg green ++ st bold) else A.fg A.red
  in

  let title = I.string title_attr "Package Details" in
  let separator =
    I.string A.empty (String.make (min 80 model.terminal_width) '-')
  in
  let empty = I.empty in

  let name_line =
    I.hcat
      [ I.string label_attr "Name: "; I.string value_attr selected_pkg.name ]
  in
  ...
  let status_text =
    if selected_pkg.installed then "Installed ✅" else "Not installed"
  in
  let status_line =
    I.hcat [ I.string label_attr "Status: "; I.string status_attr status_text ]
  in
  ...
  let help =
    I.string help_attr "Press Enter/Esc/Backspace to return | q: Quit"
  in
...
let render_list model =
  ...
  let visible_packages =
    ( model.filtered_packages |> fun l ->
      let rec drop n = function
        | [] -> []
        | _ :: tl when n > 0 -> drop (n - 1) tl
        | l -> l
      in
      drop model.scroll_offset l )
    |> fun l ->
...
let handle_key model key mods =
  match (key, mods) with
  | (`ASCII 'q' | `ASCII 'Q'), _
    when model.view_mode = List && model.search_text = "" ->
      { model with running = false }
  | `Escape, _ when model.view_mode = List && model.search_text = "" ->
      { model with running = false }
  | `Escape, _ ->
      if model.view_mode = Details then { model with view_mode = List }
      else
  ...
  | `Enter, _ ->
      let new_mode =
        match model.view_mode with List -> Details | Details -> List
      in
...
let rec event_loop term events model =
  if not model.running then Lwt.return_unit
  else
    ...
    | `Resize (width, height) ->
        let new_model =
          adjust_scroll_offset
            { model with terminal_width = width; terminal_height = height }
        in
        event_loop term events new_model
